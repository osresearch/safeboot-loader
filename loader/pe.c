...
static int pe_name_compare(const struct PeSectionHeader *sect,
                                   const char *name)
{
    size_t i;
...
void *pe_find_section(void *image, const size_t image_size,
                                   const char *section_name, size_t *size_out)
{
	...
	{
            Print(u"PE invalid section size %08x + address %016lx > size %016lx\n", sect->VirtualSize, sect->VirtualAddress, image_size);
            return NULL;
        }
